...
export async function GET(req: NextRequest) {
  try {
    ...
    const items = await db
      .select()
      .from(discordEmbedPresets)
      .where(and(eq(discordEmbedPresets.scopeType, scopeType), eq(discordEmbedPresets.scopeId, scopeId as any)));
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { name, payload, scopeType = "org", scopeId } = await req.json();
    ...
    const rows = await db
      .insert(discordEmbedPresets)
      .values({ name, payloadJson: payload, scopeType, scopeId: resolvedScopeId })
      .returning({ id: discordEmbedPresets.id });
