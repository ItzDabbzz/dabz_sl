...
import { Search, X, LayoutGrid, List as ListIcon, Loader2, Heart, Scale, Trash2, Link2 } from "lucide-react";
...
export default function ClientExplorer() {
  ...
  useEffect(() => {
    try {
      const f = JSON.parse(localStorage.getItem("mp_favorites") || "[]") as string[];
      setFavorites(new Set(f));
    } catch {}
    try {
      const c = JSON.parse(localStorage.getItem("mp_compare") || "[]") as string[];
      setCompare(new Set(c));
    } catch {}
    try {
      const r = JSON.parse(localStorage.getItem("mp_recent_searches") || "[]") as string[];
      setRecent(Array.isArray(r) ? r.slice(0, 8) : []);
    } catch {}
  }, []);

  useEffect(() => {
    try { localStorage.setItem("mp_favorites", JSON.stringify(Array.from(favorites))); } catch {}
  }, [favorites]);
  useEffect(() => {
    try { localStorage.setItem("mp_compare", JSON.stringify(Array.from(compare))); } catch {}
  }, [compare]);
  ...
  useEffect(() => {
    const t = setTimeout(() => {
      ...
      setRecent((prev) => {
        ...
        try { localStorage.setItem("mp_recent_searches", JSON.stringify(next)); } catch {}
        return next;
      });
    }, 600);
  ...
  useEffect(() => {
    function onKey(e: KeyboardEvent) {
      ...
      if (e.key === "ArrowLeft" && scrollMode === "pages") {
        const next = page - 1; if (next >= 0) fetchItems({ reset: true, pageIndex: next });
      }
      if (e.key === "ArrowRight" && scrollMode === "pages") {
        const next = page + 1; if (next * limit < total) fetchItems({ reset: true, pageIndex: next });
      }
    }
  ...
  useEffect(() => {
    ...
    const io = new IntersectionObserver((entries) => {
      ...
      if (entry.isIntersecting && !loading) {
        const next = page + 1;
        if (next * limit < total) fetchItems({ reset: false, pageIndex: next });
      }
  ...
  const visibleItems = useMemo(() => {
    let out = items;
    if (hasImageOnly) out = out.filter((it) => (it.images?.[0] ?? "").length > 0);
    if (view === "list") {
      // keep order
    }
    return out;
    ...
    <div className="grid grid-rows-[auto_1fr_auto] gap-4 h-[calc(100vh-7rem)] min-h-0">
          ...
          <div className="mx-auto w-full max-w-5xl">
            <div className="flex flex-col items-stretch gap-3 sm:flex-row sm:items-center sm:justify-center sm:gap-4">
              ...
              <div className="flex items-center gap-2 justify-center">
                ...
                <Switch checked={hasImageOnly} onCheckedChange={setHasImageOnly} />
              </div>
              ...
              <div className="flex items-center gap-2 justify-center">
                ...
                <Select value={scrollMode} onValueChange={(v: any) => setScrollMode(v)}>
              ...
              <div className="flex items-center gap-2 justify-center">
                <Button
                  ...
                  size="sm"
                  onClick={() => {
                    setQ(""); setPrimary(""); setSub(""); setPage(0); setHasImageOnly(false);
                  }}
                >
            ...
            {recent.length > 0 && (
              <div className="mt-2 flex flex-wrap items-center justify-center gap-2 text-xs">
                {recent.map((r) => (
                  <Button key={r} size="sm" variant="ghost" className="h-7 px-2" onClick={() => setQ(r)}>
                    {r}
                  </Button>
                ))}
              </div>
            ...
            {(q || primary || sub || hasImageOnly) && (
              <div className="mt-2 flex flex-wrap items-center justify-center gap-2 text-xs">
                {q ? (
                  <Button size="sm" variant="secondary" className="h-7 px-2" onClick={() => setQ("")}>q: {q} <X className="ml-1 h-3 w-3" /></Button>
                ) : null}
                {primary ? (
                  <Button size="sm" variant="secondary" className="h-7 px-2" onClick={() => { setPrimary(""); setSub(""); }}>primary: {primary} <X className="ml-1 h-3 w-3" /></Button>
                ) : null}
                {sub ? (
                  <Button size="sm" variant="secondary" className="h-7 px-2" onClick={() => setSub("")}>sub: {sub} <X className="ml-1 h-3 w-3" /></Button>
                ) : null}
                {hasImageOnly ? (
                  <Button size="sm" variant="secondary" className="h-7 px-2" onClick={() => setHasImageOnly(false)}>has image <X className="ml-1 h-3 w-3" /></Button>
                ) : null}
              </div>
            )}
            ...
            <div className="mt-2 text-center text-xs text-muted-foreground">
              {total ? (
                <span>
                  {new Intl.NumberFormat().format(total)} results • Page {page + 1} of {Math.max(1, Math.ceil(total / limit))}
                </span>
              ) : (
          ...
          <CardContent className="p-4 h-full">
            <ScrollArea className="h-full">
              ...
              {loading && page === 0 && items.length === 0 ? (
                <div className={`grid ${view === "grid" ? "grid-cols-2 sm:grid-cols-3 xl:grid-cols-4" : "grid-cols-1"} gap-4 pr-2`}>
                ...
                <div className={view === "grid" ? "grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 gap-4 pr-2" : "flex flex-col gap-3 pr-2"}>
                  {visibleItems.map((it) => {
                    ...
                    const mediaClass = view === "list" ? "h-24 w-40 sm:h-28 sm:w-44" : "aspect-video";

                    const Media = (
                      <div className={`overflow-hidden rounded bg-muted ${mediaClass}`}>
                        {it.images?.[0] && (
                          // eslint-disable-next-line @next/next/no-img-element
                          <img src={it.images[0]} alt={it.title} className="h-full w-full object-cover transition group-hover:scale-[1.02]" />
                        )}
                      </div>
                    );

                    const Meta = (
                      <div className={view === "list" ? "space-y-1" : "space-y-1 mt-2"}>
                        <div className="flex items-start justify-between gap-3 min-w-0">
                          <div className="text-sm font-medium truncate" title={it.title || "Untitled"}>{it.title || "Untitled"}</div>
                          <div className={`shrink-0 ${view === "list" ? "text-sm font-medium" : "text-xs"} text-muted-foreground`}>L$ {it.price ?? "-"}</div>
                        </div>
                        ...
                        <div className="flex flex-col items-end gap-2 min-w-[110px]">
                          <div className="flex flex-col gap-2">
                            <Button
                              ...
                              size="icon"
                              aria-label="Favorite"
                              onClick={(e) => { e.preventDefault(); const next = new Set(favorites); next.has(it.id) ? next.delete(it.id) : next.add(it.id); setFavorites(next); }}
                            >
                              <Heart className={`h-4 w-4 ${fav ? "text-pink-500" : ""}`} />
                            </Button>
                            <Button
                              variant={cmp ? "secondary" : "ghost"}
                              size="icon"
                              aria-label="Compare"
                              onClick={(e) => { e.preventDefault(); const next = new Set(compare); next.has(it.id) ? next.delete(it.id) : next.add(it.id); setCompare(next); }}
                            >
                            ...
                            <Button
                              ...
                              onClick={(e) => { e.preventDefault(); copy(it.url, "Listing link copied"); }}
                            >
                              <Link2 className="h-4 w-4" />
                            </Button>
                          </div>
                          <Button size="sm" variant="outline" className="w-24" onClick={(e) => { e.preventDefault(); setDescItem(it); }}>
                            Details
                          </Button>
                          <Button asChild size="sm" className="w-24 mt-auto">
                        ...
                        <Card key={it.id} className="group">
                          <CardContent className="p-3">
                            <div className="grid grid-cols-[10rem_1fr_auto] gap-4 items-center">
                              {Media}
                              <div className="min-w-0 flex items-center">{Meta}</div>
                              {Actions}
                            </div>
                          </CardContent>
                        </Card>
              ...
              {scrollMode === "infinite" && loading && items.length > 0 && (
      ...
      <Dialog open={!!descItem} onOpenChange={(open) => { if (!open) setDescItem(null); }}>
        <DialogContent className="max-h-[85vh] w-[99vw] sm:max-w-[1400px] xl:max-w-[1600px] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{descItem?.title || "Item details"}</DialogTitle>
            <DialogDescription>Listing description and details.</DialogDescription>
          </DialogHeader>
          <div className="space-y-3">
            {descItem?.images?.[0] ? (
              <div className="h-64 sm:h-72 md:h-80 overflow-hidden rounded bg-muted max-w-[900px] mx-auto">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={descItem.images[0]} alt={descItem.title || "Image"} className="h-full w-full object-cover" />
              </div>
            ) : null}
            <div className="flex items-center justify-between text-sm">
              <div className="font-medium truncate">{descItem?.title || "Untitled"}</div>
              <div className="text-sm text-muted-foreground">L$ {descItem?.price ?? "-"}</div>
            </div>
            <div className="text-sm whitespace-pre-wrap leading-relaxed text-muted-foreground">
              {descItem?.description || "No description provided."}
            </div>
            <div className="pt-1">
              {descItem?.url ? (
                <Button asChild size="sm">
                  <a href={descItem.url} target="_blank" rel="noopener noreferrer">Open listing</a>
                </Button>
              ) : null}
            </div>
          </div>
      ...
      <Dialog open={compareOpen} onOpenChange={setCompareOpen}>
        ...
        <DialogContent className="max-h-[85vh] w-[99vw] sm:max-w-[90vw] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>Side-by-side overview of selected items.</DialogDescription>
          </DialogHeader>
              ...
              return (
                <Card key={id} className="relative">
                  <Button
                    ...
                    onClick={() => { const next = new Set(compare); next.delete(id); setCompare(next); }}
                  >
                    <X className="h-4 w-4" />
                  </Button>
                  <CardContent className="p-4 space-y-3">
                    ...
                    {creatorName ? <div className="text-[11px] text-muted-foreground">by {creatorName}</div> : null}
