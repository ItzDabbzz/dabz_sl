...
export default function ClientExplorer() {
	...
	// Remove local items/loading/total state in favor of React Query
	...
	// React Query: shared key base
	const queryKeyBase = useMemo(
		() => ["mpItems", q.trim(), selectedCategoryId, sort, limit] as const,
		[q, selectedCategoryId, sort, limit],
	);

	// Infinite mode
	const infiniteQuery = useInfiniteQuery<ItemsResp>({
		...
		queryFn: async ({ pageParam }) => {
			...
			return (await res.json()) as ItemsResp;
		},
		getNextPageParam: (last) => {
			const nextOffset = (last?.offset ?? 0) + (last?.items?.length ?? 0);
			return nextOffset < (last?.total ?? 0) ? nextOffset : undefined;
		},
	});

	// Pages mode
	const pageQuery = useQuery<ItemsResp>({
		queryKey: [...queryKeyBase, "page", page],
		enabled: scrollMode === "pages",
	...
	}, [scrollMode, infiniteQuery.data, pageQuery.data]);
	...
	const initialLoading = useMemo(() => {
		return scrollMode === "infinite"
			? (infiniteQuery.isPending && (allItems.length === 0))
			: (pageQuery.isPending && (allItems.length === 0));
	}, [scrollMode, infiniteQuery.isPending, pageQuery.isPending, allItems.length]);

	// Keyboard shortcuts (pages mode)
	useEffect(() => {
		function onKey(e: KeyboardEvent) {
			...
			if (e.key === "ArrowRight" && scrollMode === "pages") {
				const next = page + 1;
				if (next * limit < totalCount) setPage(next);
			}
		}
		window.addEventListener("keydown", onKey);
		return () => window.removeEventListener("keydown", onKey);
	}, [page, limit, totalCount, scrollMode]);
					...
					<CardContent className="p-4 h-full">
						<ScrollArea className="h-full">
								...
								<>
									{view === "list" ? (
										<Virtuoso
											...
											components={{
												Footer: () => (
													scrollMode === "infinite" && infiniteQuery.isFetchingNextPage ? (
							...
							{scrollMode === "pages" && (
								<div className="py-2">
									<Pagination>
										<PaginationContent ref={pagListRef as unknown as React.RefObject<HTMLUListElement>}>
											<PaginationItem ref={pagPrevRef as unknown as React.RefObject<HTMLLIElement>}>
												<PaginationPrevious
													href="#"
											...
											<PaginationItem ref={pagCurrRef as unknown as React.RefObject<HTMLLIElement>}>
												<PaginationLink href="#" isActive>
													{page + 1} /{" "}
													{Math.max(1, Math.ceil(totalCount / limit))}
												</PaginationLink>
											</PaginationItem>
											<PaginationItem ref={pagNextRef as unknown as React.RefObject<HTMLLIElement>}>
												<PaginationNext
													href="#"
													onClick={(e: React.MouseEvent) => {
														e.preventDefault();
				...
				<DialogContent className="max-h-[85vh] w-[99vw] sm:max-w-[90vw] overflow-y-auto">
					...
					<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
						{Array.from(compare).map((id) => {
							const it = allItems.find((i: Item) => i.id === id);
							if (!it) return null;
							const creatorName = it.creator?.name?.trim();
							return (
