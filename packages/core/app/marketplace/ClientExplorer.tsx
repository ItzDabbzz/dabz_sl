...
export default function ClientExplorer() {
    ...
    return (
        <div className="grid grid-rows-[auto_1fr] gap-4 h-full min-h-0">
            {/* Header / Controls: Home + Search + Filters + View + Scroll + Reset + Page Nav */}
            <Card className="border bg-background/60 backdrop-blur supports-[backdrop-filter]:bg-background/40 sticky top-0 z-30">
                <CardContent className="p-3">
                    <div className="w-full">
                        ...
                        {/* Results summary with page controls */}
                        <div className="mt-2 flex items-center justify-center gap-2 text-xs text-muted-foreground">
                            {total ? (
                                scrollMode === "pages" ? (
                                    ...
                                    <span>{new Intl.NumberFormat().format(total)} results</span>
                                )
                            ) : (
                    ...
                    <CardContent className="p-4 h-full">
                        <div className="h-full">
                            ...
                            {loading && page === 0 && items.length === 0 ? (
                                <div
                                    className={`grid ${view === "grid" ? "grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 2xl:grid-cols-7" : "grid-cols-1"} gap-4`}
                                >
                                            ...
                                            itemContent={(index, it) => {
                                                ...
                                                return (
                                                    <Card key={it.id} className="group">
                                                        <CardContent className="p-3">
                                                            {/* Clickable image only (avoid nesting anchors) */}
                                                            <div className="overflow-hidden rounded bg-muted aspect-video relative">
                                                                {it.images?.[0] && (
                                                                    <a href={it.url} target="_blank" rel="noopener noreferrer" className="block h-full w-full">
                                                                        <Image src={it.images[0]} alt={it.title} className="object-cover transition group-hover:scale-[1.02]" fill sizes="(max-width: 768px) 50vw, (max-width: 1280px) 33vw, 25vw" />
                                                                    </a>
                                                                )}
                                                            </div>
                                                            <div className="space-y-1 mt-2">
                                                                <div className="flex items-start justify-between gap-3 min-w-0">
                                                                    {/* Make title its own link (sibling, not nested) */}
                                                                    <a
                            ...
                            {/* Removed bottom Pagination; moved to header */}
                        </div>
                    </CardContent>
                </Card>
            </div>
