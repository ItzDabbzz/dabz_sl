...
export default async function BlogPostPage({
    ...
    const [row] = await db
        .select({ post: blogPosts, authorName: user.name, authorImage: user.image })
        .from(blogPosts)
    ...
    return (
        <div className="relative mx-auto max-w-6xl px-6 py-10">
            ...
            <Button asChild variant="secondary" className="fixed left-4 top-1/2 hidden -translate-y-1/2 md:inline-flex">
                <Link href="/blog" aria-label="Back to all posts">
                    <ArrowLeft className="mr-2 h-4 w-4" />
                    <span className="sr-only md:not-sr-only">All Posts</span>
                </Link>
            </Button>
                ...
                <main className="lg:col-span-9 xl:col-span-9 min-w-0">
                    ...
                    <Card className="mb-6 bg-gradient-to-b from-card to-muted/40">
                        <CardContent className="p-6 md:p-8">
                            ...
                            {post.excerpt ? (
                                <p className="mt-3 text-balance text-base text-muted-foreground md:text-lg">{post.excerpt}</p>
                            ) : null}

                            <div className="mt-5 flex flex-wrap items-center gap-3">
                                <div className="flex items-center gap-3">
                                    <Avatar>
                                        <AvatarImage src={row.authorImage || undefined} alt={row.authorName || "Author"} />
                                        <AvatarFallback>{initials}</AvatarFallback>
                                    </Avatar>
                                    <div className="text-sm text-muted-foreground">
                                        <div className="font-medium text-foreground">{row.authorName || "Unknown Author"}</div>
                                        <div>
                                            {published ? published : "Unpublished"} • {readMin} min read
                                        </div>
                                    </div>
                                </div>
                                ...
                                {categories.length ? (
                                    <>
                                        ...
                                        <div className="flex flex-wrap items-center gap-2">
                                            {categories.map((c) => (
                                                <Link key={c.id} href={`/blog?c=${encodeURIComponent(c.slug)}`}>
                            ...
                            <div className="mt-6">
                                <Card>
                                    <CardContent className="flex items-center justify-between gap-4 p-4">
                                        <div className="text-sm font-medium">Rate this post</div>
