...
export async function saveSettings(formData: FormData) {
  ...
  if (!user) return;
  const heroCategoryId = String(formData.get("heroCategoryId") || "").trim() || null;
  const featuredCategoryId = String(formData.get("featuredCategoryId") || "").trim() || null;
  const seoTitleSuffix = String(formData.get("seoTitleSuffix") || "").trim() || null;
  const seoDefaultDescription = String(formData.get("seoDefaultDescription") || "").trim() || null;
  const seoOgImageUrl = String(formData.get("seoOgImageUrl") || "").trim() || null;
  ...
  const emailTemplateSubject = String(formData.get("emailTemplateSubject") || "").trim() || null;
  const emailTemplateHtml = String(formData.get("emailTemplateHtml") || "").trim() || null;
...
export default async function BlogSettingsPage() {
  const user = await getBlogEditorUser();
  if (!user) return <div className="p-6 text-sm text-red-500">You do not have access.</div>;
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Publishing & Featured</CardTitle>
        </CardHeader>
        <CardContent>
          <form action={saveSettings} className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-2 sm:col-span-1">
              <label className="text-sm font-medium">Hero Category</label>
              <select
                name="heroCategoryId"
            ...
            <div className="space-y-2 sm:col-span-1">
              ...
              <select
                ...
                defaultValue={settings?.featuredCategoryId || ""}
                className="w-full rounded border bg-background px-3 py-2"
              >
                <option value="">None</option>
                {categories.map((c) => (
            ...
            <div className="space-y-2 sm:col-span-2">
              <label className="text-sm font-medium">Enable Ratings Summary</label>
              <div className="flex items-center gap-2">
                <input
                  type="checkbox"
                  name="enableRatingsSummary"
                  defaultChecked={settings?.enableRatingsSummary ?? true}
                />
            ...
            <div className="space-y-2 sm:col-span-2">
              <label className="text-sm font-medium">SEO Title Suffix</label>
              <Input
                name="seoTitleSuffix"
            ...
            <div className="space-y-2 sm:col-span-2">
              <label className="text-sm font-medium">Default Open Graph Image URL</label>
              <Input
                name="seoOgImageUrl"
            ...
            <div className="space-y-2 sm:col-span-2">
              <label className="text-sm font-medium">Email on First Publish</label>
              <div className="flex items-center gap-2">
                <input
                  type="checkbox"
                  name="enableEmailOnPublish"
                  defaultChecked={settings?.enableEmailOnPublish ?? false}
                />
                <span className="text-sm text-muted-foreground">
                  Allow sending a one-time announcement per post
                </span>
              </div>
            </div>
            ...
            <div className="sm:col-span-2">
              <EmailTemplateEditor
                initialSubject={settings?.emailTemplateSubject || "New post: {{title}}"}
      ...
      <Card>
        ...
        <CardContent>
          <div className="text-sm text-muted-foreground">
            Total ratings:{" "}
            <span className="text-foreground font-medium">{ratingSummary?.count ?? 0}</span> · Average score:{" "}
