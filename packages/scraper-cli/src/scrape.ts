...
import { resolveOut, saveJSON } from "./utils";
...
function deriveBaseName(urls: string[], argv: any): string {
...
type Permissions = { copy: string; modify: string; transfer: string };
...
type Item = {
  ...
  contents: string[];
  updatedOn: string;
  categories?: Array<{ primary: string; sub: string }>; // optional categorization
};
...
async function configurePage(page: Page, argv: any) {
  try {
    page.setDefaultTimeout(argv.timeout);
    page.setDefaultNavigationTimeout(argv["nav-timeout"]);
    if (argv["block-assets"]) {
...
async function scrapeItem(page: Page, url: string): Promise<Item> {
  ...
  try {
    ...
    if (hasTitle2) {
      title = (await page.locator("h1.title2").first().evaluate((el) => {
        const clone = el.cloneNode(true) as HTMLElement;
        const v = clone.querySelector(".version");
        if (v) v.remove();
  ...
  if (!title) {
    title = (await page.locator('h1[itemprop="name"], h1.product-title, h1').first().textContent().catch(() => "") || "").trim();
  }
  if (!version) {
    const versionText = await page.locator("text=/Version:\\s*/i").first().textContent().catch(() => "");
    if (versionText) {
      const m = versionText.match(/Version:\s*(.*)/i);
      if (m) version = m[1].trim();
    } else {
      const details = await page.locator(".product-details, .details, table").first();
      const html = await details.innerText().catch(() => "");
      const m = html.match(/Version\s*:?\s*(.+)/i);
  ...
  const images: string[] = [];
  try {
    const imgSel = "section.listing__image .product-image .main-image img, section.listing__image #image-thumbnails img, img[itemprop=\"image\"], .product-images img, .carousel img";
    const srcs = await page.$$eval(imgSel, (els) => (els as HTMLImageElement[]).map((e) => e.getAttribute("src")).filter(Boolean) as string[]);
    const base = page.url();
  ...
  let price = "";
  const priceTextNew = (await page.locator('.prices .price-ld, .linden-price, [itemprop="price"], .price').first().innerText().catch(() => "")).trim();
  if (priceTextNew) {
    const m = priceTextNew.match(/([\d,]+)/);
  ...
  if (!creator.name || !creator.link) {
    const creatorAnchor = page.locator('a[href*="/stores/"] , a[href*="/p/" i]:has-text("by ")').first();
    const storeAnchor = page.locator('a[href*="/stores/"] .store-name, a[href*="/stores/"]');
    try {
      if (await creatorAnchor.count()) {
        const link = await creatorAnchor.getAttribute("href");
        const text = (await creatorAnchor.innerText()).replace(/^by\s+/i, "").trim();
        creator = { name: text, link: new URL(link || "", page.url()).toString() };
      }
    } catch {}
    try {
      if (await storeAnchor.count()) {
        const first = storeAnchor.first();
  ...
  try {
    const lis = await page.$$eval("#permissions li", (els) => (els as HTMLElement[]).map((li) => ({
      text: (li.textContent || "").trim(),
      cls: (li as HTMLElement).className || ""
    })));
    for (const li of lis as { text: string; cls: string }[]) {
  ...
  if (!perms.copy && !perms.modify && !perms.transfer) {
    const permsText = await page.locator("text=Permissions, .permissions, .product-permissions").first().locator("..").innerText().catch(async () => await page.locator(".permissions, .product-permissions").first().innerText().catch(() => ""));
    if (permsText) {
  ...
  const description = (await page.locator("#product-description").first().innerText().catch(() => "")).trim() ||
    (await page.locator(".product-description").first().innerText().catch(() => "")).trim();

  const featuresBlock = await page.locator("#product-features, .features").first().innerText().catch(() => "");
  const features = featuresBlock ? featuresBlock.split(/\n+/).map(s => s.replace(/^[−•\s-]+/, "").trim()).filter(Boolean) : [];

  const contentsBlock = await page.locator("#product-contents, .contents").first().innerText().catch(() => "");
  const contents = contentsBlock ? contentsBlock.split(/\n+/).map(s => s.replace(/^[−•\s-]+/, "").trim()).filter(Boolean) : [];

  let updatedOn = "";
  try {
    const upd = (await page.locator(".section__rating .update-block").first().innerText({ timeout: 1000 }).catch(() => "")) || "";
    const m = upd.match(/Updated\s*on\s*:?\s*(.+)/i) || upd.match(/Last\s*Updated\s*:?\s*(.+)/i) || upd.match(/Updated:\s*(.+)/i);
    if (m) updatedOn = m[1].split("\n")[0].trim();
  } catch {}
  if (!updatedOn) {
    const metaText = await page.locator(".product-metadata, .meta, .details, .product-details").first().innerText().catch(() => "");
    const mUpd = metaText.match(/Updated\s*on\s*:?\s*(.+)/i) || metaText.match(/Last\s*Updated\s*:?\s*(.+)/i) || metaText.match(/Updated:\s*(.+)/i);
    if (mUpd) updatedOn = mUpd[1].split("\n")[0].trim();
  }
...
function buildCatalog(items: Item[]) {
  const catalog: Record<string, Record<string, Item[]>> = {};
...
(async function main() {
  ...
  if (argv.sub && argv.sub.length) {
    ...
    for (const spec of specs) {
      ...
      try {
        ...
        console.error(kleur.yellow(`Failed to read sub list ${filePath}: ${e.message}`));
      }
    }
  }
  ...
  if (argv.input) {
    ...
    for (const inp of inputs) {
      ...
      catch (e: any) { console.error(kleur.yellow(`Warning: failed to read ${inp}: ${e.message}`)); }
    }
  }
  if (argv.url) {
  ...
  try {
    ...
    const computeTokens = () => ({
      active: activeByWorker.filter(Boolean).length,
      now: (activeByWorker.find(Boolean) || '').slice(0, 70)
    });

    const bar = new SingleBar({
      format: `${kleur.blue('queue')} [{bar}] {value}/{total} | ${kleur.green('ok')}:{ok} ${kleur.red('fail')}:{fail} ${kleur.yellow('active')}:{active} ${kleur.gray('| now:')} {now}`,
      hideCursor: true,
    ...
    async function worker(wid: number) {
      while (true) {
        ...
        try {
          ...
          await configurePage(page, argv);
          const item = await scrapeItem(page, url);
          item.categories = urlToCats.get(url) || [];
          items[i] = item;
          ok++; processed++;
          ...
          failures.push({ url, err: e && e.message ? e.message : String(e) });
          fail++; processed++;
          activeByWorker[wid - 1] = '';
          bar.update(processed, { ok, fail, ...computeTokens() });
          if (!pretty || verbose) console.log(kleur.red(`[w${wid}] ✗ ${url} — ${e.message || e}`));
        } finally {
    ...
    const cleanItems = items.filter(Boolean) as Item[];
